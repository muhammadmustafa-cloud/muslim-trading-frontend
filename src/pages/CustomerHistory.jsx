      ...
      <section className="card p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
            ...
            <div className={`text-2xl font-black ${data.summary?.finalBalance >= 0 ? 'text-blue-600' : 'text-red-600'}`}>
              Rs. {formatMoney(Math.abs(data.summary?.finalBalance || 0))}
              <span className="text-xs ml-1 font-bold">
                {data.summary?.finalBalance > 0 ? '(Receivable)' : data.summary?.finalBalance < 0 ? '(Payable)' : ''}
              </span>
            </div>
          </div>
        </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {data.ledger.map((item, idx) => (
                    <tr key={idx} className="table-row-hover border-b border-slate-100 italic-last-row">
                      ...
                      <td className={`table-cell text-right font-bold bg-slate-50/50 ${item.balance >= 0 ? 'text-blue-800' : 'text-red-800'}`}>
