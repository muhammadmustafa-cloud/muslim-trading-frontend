...
const formatDay = (d) => (d ? new Date(d).toLocaleDateString("en-PK", { weekday: "long", timeZone: "Asia/Karachi" }) : "—");
...
export default function CustomerHistory() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="page-title flex items-center gap-2">
              ...
              Khatta (History) — {data.name || "Customer"}
            </h1>
      ...
      <section className="card p-5 bg-white shadow-soft border-l-4 border-l-indigo-500">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="md:col-span-2">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div>
                ...
                <input type="date" value={dateFrom} onChange={(e) => setFilters({ dateFrom: e.target.value })} className="input-field border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 italic">Ikhtatam (To)</label>
                <input type="date" value={dateTo} onChange={(e) => setFilters({ dateTo: e.target.value })} className="input-field border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
              </div>
          ...
          <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 shadow-xl flex flex-col justify-center transform hover:scale-[1.02] transition-transform">
            <h3 className="text-[10px] font-black text-indigo-400 uppercase mb-1 tracking-widest">NET RECEIVABLE (Closing)</h3>
            <div className={`text-2xl font-black ${data.summary?.finalBalance >= 0 ? 'text-blue-400' : 'text-rose-400'}`}>
              Rs. {formatMoney(Math.abs(data.summary?.finalBalance || 0))}
            </div>
            <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">
               {data.summary?.finalBalance > 0 ? 'Customer Khata (Receivable)' : data.summary?.finalBalance < 0 ? 'Mill Khata (Payable)' : 'Pura Hisaab (Settled)'}
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="overflow-x-auto">
            {!data.ledger?.length ? (
              <div className="p-20 text-center text-slate-400">
                 <FaUsers className="w-16 h-16 mx-auto mb-4 opacity-10" />
                 <p className="font-bold tracking-widest uppercase text-xs">No entries found for this customer.</p>
              </div>
            ) : (
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-800 text-white border-b border-slate-700">
                    <th className="py-4 px-3 text-left text-[10px] font-black uppercase tracking-widest">Date</th>
                    <th className="py-4 px-2 text-left text-[10px] font-black uppercase tracking-widest">Day</th>
                    <th className="py-4 px-4 text-left text-[10px] font-black uppercase tracking-widest">Description</th>
                    <th className="py-4 px-3 text-center text-[10px] font-black uppercase tracking-widest">Bag</th>
                    <th className="py-4 px-3 text-center text-[10px] font-black uppercase tracking-widest">Rate</th>
                    <th className="py-4 px-3 text-center text-[10px] font-black uppercase tracking-widest">Payment Due Date</th>
                ...
                <tbody className="divide-y divide-slate-100 italic font-medium">
                  {data.ledger.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-50 transition-colors border-b border-slate-100 italic-last-row">
                      <td className="py-4 px-3 whitespace-nowrap text-[11px] font-bold text-slate-400">{formatDate(item.date)}</td>
                      <td className="py-4 px-2 whitespace-nowrap text-[11px] font-bold text-slate-400">{formatDay(item.date)}</td>
                      ...
                      <td className="py-4 px-3 text-center text-xs font-bold text-slate-600">{item.rate || '—'}</td>
                      <td className="py-4 px-3 text-center text-xs font-bold text-slate-500">{formatDate(item.dueDate)}</td>
                      ...
                      <td className={`py-4 px-4 text-right font-black text-base shadow-inner ${item.balance >= 0 ? 'text-blue-800 bg-blue-50/30' : 'text-rose-800 bg-rose-50/30'}`}>
                        ...
                        <span className="text-[10px] ml-1 uppercase">{item.balance >= 0 ? 'Dr' : 'Cr'}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
                <tfoot className="bg-slate-800 text-white font-black">
                  <tr>
                    ...
                    <td className="py-6 px-4 text-right bg-slate-900 border-t-4 border-amber-500 text-xl font-black">
                      ...
                      <span className="text-[11px] ml-2 text-slate-400 uppercase tracking-widest">{data.summary?.finalBalance >= 0 ? 'Receivable (Dr)' : 'Payable (Cr)'}</span>
                    </td>
                  </tr>
                </tfoot>
              </table>
