 ...
 */
export default function PayBillModal({ entry, open, onClose, onSuccess }) {
    ...
    useEffect(() => {
        if (open) {
            fetchAccounts();
            const remaining = entry?.amount - (entry?.amountPaid || 0);
            setAmount(remaining > 0 ? remaining : '');
            setNote(`Payment for Bill: ${entry?.itemId?.name} | Date: ${new Date(entry?.date).toLocaleDateString()} ${entry?.truckNumber ? `| Truck: ${entry.truckNumber}` : ''}`);
            setError('');
        }
    }, [open, entry]);
    ...
    const handleSubmit = async (e) => {
        ...
        const balance = entry.amount - (entry.amountPaid || 0);
        ...
        setError('');
        try {
            const response = await apiPost(`/stock-entries/${entry._id}/pay`, {
                amount: payAmt,
                accountId,
                date,
                note
    ...
    return (
        <Modal open={open} onClose={onClose} title="Settle Bill (Payment Karein)">
            <div className="mb-4 p-3 bg-slate-50 border border-slate-200 rounded-lg">
                <div className="flex justify-between items-center mb-1">
                    <span className="text-xs text-slate-500 font-medium uppercase tracking-wider">Bill Reference</span>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div><p className="text-slate-500">Total:</p><p className="font-semibold text-slate-900">{entry.amount.toLocaleString()}</p></div>
                    <div><p className="text-slate-500">Remaining:</p><p className="font-bold text-red-600">{remaining.toLocaleString()}</p></div>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    <label className="input-label">Account (Paise kahan se jaenge?) *</label>
                ...
                <div>
                    <label className="input-label">Audit Note (Professional Reference)</label>
                ...
                <div className="flex gap-2 pt-2">
                    <button type="submit" disabled={loading} className="btn-primary flex-1 py-3 text-lg flex items-center justify-center gap-2">
                        <FaMoneyBillWave /> {loading ? 'Processing...' : 'Konfirm Payment'}
                    </button>
